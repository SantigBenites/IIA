...
def search(agents, world, verbose = False, step_by_step = False):
    path_seq = dict()
    pathcost = dict()
    agent_goal = dict()
    max_pathlen = 0
    ...
    if verbose:
        print("Current plans:")
        for agent in agents:
            print(" for agent ", agent, ": ", path_seq[agent])
        if step_by_step:
    ...
    while(True): # iter_count < 5):
            ...
            if (agent in conflicts_db):
                ...
                if(bool(constraints)):
                    ...
                    if verbose:
                        ...
                        print(' Start:',start, ' Target:', goal, ' Constraints: ', constraints)
                        print(' New Path: ', nw_path)
                        if step_by_step:
                            something = input("Press enter to continue ...")
                            print()
        ...
        for agent in agents:
            ...
            if(ubrokn_conflicts):
                if verbose:
                    print('## for agent ', agent, ' unbroken conflicts:', ubrokn_conflicts)
    ...
    return path_seq, pathcost, conflicts_db
