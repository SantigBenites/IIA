...
def breadth_first_graph_search_plus(problem,display=False):
    """[Figure 3.11]"""
    node = Node(problem.initial)
    visited = set()
    ...
    if problem.goal_test(node.state):
        if display:
            print(len(visited), "paths have been expanded and", len(frontier), "paths remain in the frontier")
        return node
    frontier = deque([node])
    while frontier:
        node = frontier.popleft()
        if display and node.depth > level:
            level+=1
            print('Level:',level)
        for child in node.expand(problem):
                if problem.goal_test(child.state):
...
def best_first_graph_search_plus(problem, f,display=False):
    """Search the nodes with the lowest f scores first.
    You specify the function f(node) that you want to minimize; for example,
    if f is a heuristic estimate to the goal, then we have greedy best
    first search; if f is node.depth then we have breadth-first search.
    ...
    frontier.append(node)
    explored = set()
    #print('INITIAL Explored:')
    #print(explored)
    visited_not_explored = {problem.initial}
    #print('INITIAL Visited and Not Explored:')
    #print(visited_not_explored)
    level = 1
    while frontier:
        node = frontier.pop()
        if display and node.depth > level:
        ...
        #print('--- Visited and Not Explored:')
        #print(visited_not_explored)        
        for child in node.expand(problem):
...
def uniform_cost_search_plus(problem, display=False):
    """[Figure 3.14]"""
    return best_first_graph_search_plus(problem, lambda node: node.path_cost, display)
...
def astar_search_plus(problem, h=None, display=False):
    """A* search is best-first graph search with f(n) = g(n)+h(n).
    You need to specify the h function when you call astar_search, or
    else in your Problem subclass."""
    h = memoize(h or problem.h, 'h')
    return best_first_graph_search_plus(problem, lambda n: n.path_cost + h(n), display)
