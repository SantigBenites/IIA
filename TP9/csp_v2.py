...
class CSP(search.Problem):
    ...
    def suppose(self, var, value):
        ...
        When it is assumed var=val val will be the only element in its domain and all other values from var domain will be in removals."""
...
def forward_checking(csp, var, value, assignment, removals, verbose = False):
    """Suppose we assign var to value, all other values in var domain will be in removals, 
    then prune neighbor values not yet assigned inconsistent with var=value.
        Sara C. Madeira - added verbose"""
    if (verbose):
...
def backtracking_search(csp,
    ...
    def backtrack(assignment):
        ...
        for value in vals:
            ...
            if 0 == csp.nconflicts(var, value, assignment):  # with inference != None we should skip this...
                ...
                if inference(csp, var, value, assignment, removals, verbose):
                    ...
                    if result:
                        ...
                        print('\n---Backtrack on',var,'=',value)
                csp.restore(removals)
                if verbose:
                    csp.curr_domains[var]=posfix(vals,value)
                    print('Restored domains:',csp.curr_domains)
            elif (verbose):
                print("---Conflict!! Backtrack")
        if verbose:
